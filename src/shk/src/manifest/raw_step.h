...
/**
 * A RawStep is a dumb data object that represents one build statment in the
 * build manifest.
 *
 * Parsing the build manifest and evaluating the rules results in a list of
 * RawStep objects. When the RawStep object have been created, the manifest and
 * the variable environments etc can be discarded. It is not possible to
 * recreate the manifest from the list of steps; RawStep objects contain already
 * evaluated commands.
 */
struct RawStep {
  /**
   ...
   * completed.
   *
   * Empty string indicates that there is no depfile.
   */
  std::string depfile;
