...
| <a name="2.11">11</a>     | Calling `OnSubscribe`, `OnNext`, `OnError` or `OnComplete` MUST [return normally](#term_return_normally). The only legal way for a `Subscriber` to signal failure is by cancelling (or, equivalently, destroying) its `Subscription`. In the case that this rule is violated, any associated `Subscription` to the `Subscriber` MUST be considered as cancelled, and the caller MUST raise this error condition in a fashion that is adequate for the runtime environment. |
...
| <a name="3.14">14</a>     | While the `Subscription` is not cancelled, `Subscription.Cancel` (and destroying the `Subscription`) MAY cause the `Publisher`, if stateful, to transition into the `shut-down` state if no other `Subscription` exists at this point [see [1.9](#1.9)]. |
